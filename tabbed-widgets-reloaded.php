...
/*
 ...
 Thanks for suggestions to Ronald Huereca and Anupam Saha.
 */

/*
 Various users have reported that the "tabbed-widget" plugin breaks on Wordpress 3.3.1 
 and added a work around in some way to make it work. I've changed the plugin code 
 in order to make it work for Wordpress 3.3.1 and also added a functionality 
 to add custom CSS so the core CSS does not require a update.
 
 Many thanks to Ronald Huereca for developping the wonderful plugin.
 
 Main Author's URI: http://konstruktors.com/blog/
 Main Plugin URI: http://konstruktors.com/projects/wordpress-plugins/tabbed-accordion-widgets/ 
 */
...
new tabbedWidgetsReloaded();

class tabbedWidgetsReloaded {
      ...
      foreach ($sidebars_widgets as $sidebar_id => $widgets) {
        if (!empty($widgets) && $sidebar_id != 'wp_inactive_widgets') {
          foreach ($widgets as $no => $widget_id) {
            ...
            if (strpos($widget_id, 'tabbed-widgets-reloaded') === false) {
  ...
  function addAdminCSS() {
    wp_enqueue_style('tabbed-widgets-reloaded-admin', $this->plugin_path . 'css/admin-style.css');
  }
  
  function addHeader() {
    wp_enqueue_script('jquery-ui-custom',  $this->plugin_path . 'js/jquery-ui-custom.min.js', array('jquery'), false, true);
    wp_enqueue_script('jquery-ui-cookie',  $this->plugin_path . 'js/jquery-cookie.min.js', array('jquery-ui-custom'), false, true);
    ...
    $user_style = '/tabbed-widgets-reloaded/tabbed-widgets.css';
    $deps = array();
    $version = '0.1';
    if ( file_exists ( get_stylesheet_directory() . $user_style ) ) { // Child Theme (or just theme)

      wp_enqueue_style ( 'tabbed-widgets-reloaded', get_stylesheet_directory_uri() . $user_style, $deps, $version );

    } elseif ( file_exists ( get_template_directory() . $user_style ) ) { // Parent Theme (if parent exists)

      wp_enqueue_style ( 'tabbed-widgets-reloaded', get_template_directory_uri() . $user_style, $deps, $version );

    } else { //Default file in plugin folder

      // Add default widgets styles
      wp_enqueue_style('tabbed-widgets-reloaded', $this->plugin_path . 'css/tabbed-widgets.css', $deps, $version);

      if (get_current_theme() == 'Twenty Ten')
        wp_enqueue_style('tabbed-widgets-reloaded-2010', $this->plugin_path . 'css/twenty-ten.css', $deps, $version);
...
class tabbedWidgetWidget extends WP_Widget {
  ...
  function tabbedWidgetWidget() {
    $widget_ops = array('classname' => 'tabbed-widget-reloaded', 'description' => 'Place widgets inside a tabbed or an accordion type interface');
    $control_ops = array('width' => 390, 'id_base' => 'tabbed-widget-reloaded');
    $this->WP_Widget('tabbed-widget-reloaded', 'Tabbed Widget', $widget_ops, $control_ops);
  ...
  function print_tabbed_widget($args, $instance) {
    ...
    $widget_no = str_replace('tabbed-widget-reloaded-', '', $args['widget_id']);
  ...
  function makeSingleWidgetsList($instance, $inst_name) {
    ...
    if (!empty($this->active_widgets)) {
      foreach ($this->active_widgets as $widget_id => $widget_data) {
        ...
        if (strpos($widget_id, 'tabbed-widget-reloaded') === false) {
  ...
  function makeDonate() {
    return '<p class="tw-donate"><a href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=anupamsahain%40gmail%2ecom&lc=US&item_name=Tabbed%20Widgets%20Reloaded%20Plugin%20for%20WordPress&no_note=0&currency_code=USD&bn=PP%2dDonationsBF%3abtn_donateCC_LG%2egif%3aNonHostedGuest" title="Show your love and support -- make a donation. It is truly appreciated!" target="_blank">Donate</a></p>';
  }
